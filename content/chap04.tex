...
\subsection{编码重传包}
考虑到重传序列的最后一个重传包丢失造成的性能损失比较大，本文对重传报文进行再编码。在重传了\emph{re\_list}的所有不编码原始报文后，再重传$\left\lceil {R \times num\_re} \right\rceil  - num\_re$个编码报文，其中\emph{R}表示冗余度，\emph{num\_re}表示\emph{re\_list}中重传报文的个数。重传编码报文的\emph{报文状态}域填写方式参照表\ref{tab:BWZT}。优先对\emph{re\_list}链表中排在前面，也就是序号小的报文进行编码，窗口大小和正常编码窗口大小一样都为\emph{W}。这样处理的目的是为了降低解码端的解码时延。
\begin{figure}[htbp]
	\centering
